...
class HotelGuests(Document):
	...
	def on_update(self):
		if frappe.db.get_single_value("Nazeel Settings", "link_customer_to_guest"):
			if self.customer:
				if self.flags.existing_customer or frappe.db.exists(
					{"doctype": "Hotel Guests", "name": ["!=", self.name], "customer": self.customer}
				):
	...
	def set_contact(self):
		...
		if contact:
			...
			if old_doc.email != self.email or old_doc.contact_no != self.contact_no or old_doc.work_phone_no != self.work_phone_no:
			...
			if contact:
				...
				self.reload()
				if self.email or self.contact_no or self.work_phone_no:
					...
					contact.append("links", dict(link_doctype="Hotel Guests", link_name=self.name))
					if self.customer:
	...
	def update_contact(self, contact):
		...
		if self.contact_no and self.contact_no != contact.mobile_no:
			for mobile in contact.phone_nos:
				mobile.is_primary_mobile_no = True if mobile.phone == self.contact_no else False
			contact.add_phone(self.contact_no, is_primary_mobile_no=True)
			contact.set_primary("mobile_no")

		if self.work_phone_no and self.work_phone_no != contact.phone:
			for phone in contact.phone_nos:
				phone.is_primary_phone = True if phone.phone == self.work_phone_no else False
			contact.add_phone(self.work_phone_no, is_primary_phone=True)
...
def create_customer(doc):
	...
	frappe.db.set_value("Hotel Guests", doc.name, "customer", customer.name)
	frappe.msgprint(_("Customer {0} created and linked to Guest").format(customer.name), alert=True)
